...
// Импортируем глобальный контекст, где хранится наш rootStore с userStore
...
export default function AuthDialog({ open, onOpenChange, defaultTab = "login" }: AuthDialogProps) {
  ...
  const onLoginSubmit = async (values: z.infer<typeof loginSchema>) => {
    await userStore.loginUser(values.email, values.password)
      ...
      .catch((error: any) => {
        ...
        // Если сервер вернул детальное сообщение об ошибке
        if (error.response && error.response.data && error.response.data.message) {
          message = error.response.data.message;
        } else if (error.message) {
          // Если нет, то используем то, что есть в error.message
  ...
  const onRegisterSubmit = async (values: z.infer<typeof registrationSchema>) => {
    try {
      // Вызываем метод регистрации из userStore
                      ...
                      <FormItem>
                        ...
                        <div className="relative">
                          <FormControl>
                            <Input
                              placeholder="your@email.com"
                              error={!!registrationForm.formState.errors.email}
                              {...field}
                            />
                          </FormControl>
